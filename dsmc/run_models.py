...
import settings
from run_prognostics.prognostic_models import *
...
def prognostics(technique, n_clusters):
    prognostic_models = PrognosticModels(
        "results/" + technique + "/clusters/", technique, n_clusters=n_clusters
    )
    prognostic_models.estimate_rul_per_model()
    return prognostic_models


def kaplan_reliability_mortality(technique, save=True):
    visualize.kaplan_meier(technique, save_fig=save)
    visualize.reliability_mortality_rate(technique, save_fig=save)


def compare_models(prognostic_models, technique, save=True):
    if technique == "mimic":
        lengths = [1, 25, 50, 70]  # at 1, 25, 50, and 70 hours
        (
            dc_preds,
            sofa_preds,
            saps_preds,
            apache_preds,
            y_true,
        ) = utils.calculate_probas_mimic(lengths)
        visualize.auc_curve(
            y_true,
            dc_preds,
            sofa_preds,
        ...
        (
            ...
            lower_ruls_hmm,
            upper_ruls_hmm,
        ) = prognostic_models.return_results()
        visualize.compare_plot_rul(
            rmse_gb,
            ruls_gb,
            true_ruls,
            lower_ruls_gb,
...
def plot_rul_and_compare_models(prognostic_models, path, technique, save=True):
    (
        rmse_gb,
        ruls_gb,
        true_ruls,
