...
def run_process():
    ...
    # Run the prognostics model
    if TECHNIQUE == "cmaps":
        message = "C-MAPSS dataset, it needs approx. 30-50 iterations"
    elif TECHNIQUE == "mimic":
        message = "MIMIC-III dataset, it needs approx. 5-15 iterations"
    else:
        message = "F-MOC dataset, it needs approx. 10-20 iterations"

    print(f"\nRunning the prognostics model (for {message} to converge)\n")
    prognostic_models = run_models.prognostics(TECHNIQUE, n_clusters)

    if not TECHNIQUE == "fmoc":
        # Create kaplan-meier and reliability/mortality
        run_models.kaplan_reliability_mortality(TECHNIQUE, save=SAVE)

        # Calculate the metrics and compare with the baseline
        print("\nComparing the models")
        run_models.compare_models(prognostic_models, TECHNIQUE, save=SAVE)
    else:
        run_models.plot_rul_and_compare_models(
            prognostic_models,
            "results/" + TECHNIQUE + "/clusters/",
            TECHNIQUE,
            save=SAVE,
        )
